...
public class Connection : MonoBehaviour {
	...
	const int maxResolution = 2000;
	const int avgResolution = 250;
	...
	private void Start() {
		gameObject.GetComponent<LineRenderer>().SetWidth(3f, 3f);
	}
	...
	public bool isTargetZero (RectTransform transform, ConnectionPoint.ConnectionDirection dir) {
		if (!transform) return false;

		return transform.Equals (target[0]) && points[0].direction.Equals (dir);
	}
	public bool isTargetOne (RectTransform t, ConnectionPoint.ConnectionDirection dir) {
		if (!transform) return false;
		
		return t.Equals (target[1]) && points[1].direction.Equals (dir);
	}
	...
	public bool EqualsTarget(RectTransform transform ,int i){
		return transform.Equals (target[i]);
	}
