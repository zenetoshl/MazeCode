...
namespace RoslynCSharp {
    ...
    public class ScriptDomain : IDisposable {
        ...
        public ScriptType CompileAndLoadMainSource (string cSharpSource, ScriptSecurityMode securityMode = ScriptSecurityMode.EnsureLoad, IMetadataReferenceProvider[] additionalReferenceAssemblies = null) {
        ...
        public ScriptAssembly CompileAndLoadSource (string cSharpSource, ScriptSecurityMode securityMode = ScriptSecurityMode.UseSettings, IMetadataReferenceProvider[] additionalReferenceAssemblies = null) {
            ...
            lock (this) {
                ...
                Debug.Log (compileResult.OutputAssembly == null);
                // Log to console
        ...
        public void InitializeCompilerService () {
            ...
            if (sharedCompiler == null) {
                // Create the compiler
                sharedCompiler = new RoslynCSharpCompiler (false, true, Microsoft.CodeAnalysis.OutputKind.DynamicallyLinkedLibrary, Microsoft.CodeAnalysis.CSharp.LanguageVersion.Default, sandbox);
        ...
        public void ApplyCompilerServiceSettings () {
            ...
            foreach (string reference in settings.References) {
                if (Application.platform == RuntimePlatform.Android) {
                    ...
                    while (!reader.isDone) { }
                    sharedCompiler.ReferenceAssemblies.Add (AssemblyReference.FromImage (reader.bytes));
                    Debug.Log ("foi");
                } else {
