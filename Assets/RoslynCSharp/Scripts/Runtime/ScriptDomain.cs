...
namespace RoslynCSharp
{
    ...
    public class ScriptDomain : IDisposable
    {
        ...
        private SecurityReport securityResult = null;
        ...
        /// </summary>
        public SecurityReport SecurityResult
        {
        ...
        public ScriptAssembly LoadAssembly(string fullPath, ScriptSecurityMode securityMode = ScriptSecurityMode.UseSettings)
        {
            ...
            // Load the assembly
            Assembly assembly = sandbox.Load(fullPath);

            // Create script assembly
            return RegisterAssembly(assembly, securityMode, false);
        }
        ...
        public ScriptType CompileAndLoadMainSource(string cSharpSource, ScriptSecurityMode securityMode = ScriptSecurityMode.UseSettings)
        {
            return CompileAndLoadSource(cSharpSource, securityMode)?.MainType;
        }
        ...
        public ScriptType CompileAndLoadMainFile(string cSharpFile, ScriptSecurityMode securityMode = ScriptSecurityMode.UseSettings)
        {
            return CompileAndLoadFile(cSharpFile, securityMode)?.MainType;
        }
        ...
        public ScriptAssembly CompileAndLoadSource(string cSharpSource, ScriptSecurityMode securityMode = ScriptSecurityMode.UseSettings)
        {
            ...
            {
                // Compile from source
                compileResult = sharedCompiler.CompileFromSource(cSharpSource);

                // Log to console
                LogCompilerOutputToConsole();

                // Security check
                return RegisterAssembly(compileResult.OutputAssembly, securityMode, true);
            }
        }
        ...
        /// <param name="securityMode">The code validation used to verify the code</param>
        /// <returns>The compiled and loaded assembly or null if the compile or security verification failed</returns>
        public ScriptAssembly CompileAndLoadFile(string cSharpFile, ScriptSecurityMode securityMode = ScriptSecurityMode.UseSettings)
        {
            // Make sure the compiler is initialized and the domain is valid
            CheckDisposed();
            CheckCompiler();
            ...
            {
                // Compile from file
                compileResult = sharedCompiler.CompileFromFile(cSharpFile);
        ...
        /// <param name="securityMode">The code validation used to verify the code</param>
        /// <returns>The compiled and loaded assembly or null if the compile or security verification failed</returns>
        public ScriptAssembly CompileAndLoadSources(string[] cSharpSources, ScriptSecurityMode securityMode = ScriptSecurityMode.UseSettings)
        {
            // Make sure the compiler is initialized and the domain is valid
            CheckDisposed();
            CheckCompiler();
            ...
            {
                // Compile from source
                compileResult = sharedCompiler.CompileFromSources(cSharpSources);
        ...
        public ScriptAssembly CompileAndLoadFiles(string[] cSharpFiles, ScriptSecurityMode securityMode = ScriptSecurityMode.UseSettings)
        {
            // Make sure the compiler is initialized and the domain is valid
            CheckDisposed();
            CheckCompiler();
            ...
            {
                // Compile from file
                compileResult = sharedCompiler.CompileFromFiles(cSharpFiles);
        ...
        public AsyncCompileOperation CompileAndLoadSourceAsync(string cSharpSource, ScriptSecurityMode securityMode = ScriptSecurityMode.UseSettings)
        {
            // Make sure the compiler is initialized and the domain is valid
            CheckDisposed();
            CheckCompiler();

            return new AsyncCompileOperation(this, true, securityMode, cSharpSource);
        }
        ...
        /// <param name="securityMode">The code validation used to verify the code</param>
        /// <returns>An awaitable async operation object containing state information about the compile request</returns>
        public AsyncCompileOperation CompileAndLoadFileAsync(string cSharpFile, ScriptSecurityMode securityMode = ScriptSecurityMode.UseSettings)
        {
            // Make sure the compiler is initialized and the domain is valid
            CheckDisposed();
            CheckCompiler();

            return new AsyncCompileOperation(this, false, securityMode, cSharpFile);
        }
        ...
        public AsyncCompileOperation CompileAndLoadSourcesAsync(string[] cSharpSources, ScriptSecurityMode securityMode = ScriptSecurityMode.UseSettings)
        {
            // Make sure the compiler is initialized and the domain is valid
            CheckDisposed();
            CheckCompiler();

            return new AsyncCompileOperation(this, true, securityMode, cSharpSources);
        }
        ...
        /// <param name="securityMode">The code validation used to verify the code</param>
        /// <returns>An awaitable async operation object containing state information about the compile request</returns>
        public AsyncCompileOperation CompileAndLoadFilesAsync(string[] cSharpFiles, ScriptSecurityMode securityMode = ScriptSecurityMode.UseSettings)
        {
            // Make sure the compiler is initialized and the domain is valid
            CheckDisposed();
            CheckCompiler();

            return new AsyncCompileOperation(this, false, securityMode, cSharpFiles);
        }
        #endregion
        ...
        public void ApplyCompilerServiceSettings()
        {
            ...
            sharedCompiler.References.Clear();
            foreach (string reference in settings.References)
                sharedCompiler.References.Add(reference);
        ...
        private ScriptAssembly RegisterAssembly(Assembly assembly, ScriptSecurityMode securityMode, bool isRuntimeCompiled)
        {
            // Check for error
            if (assembly == null)
                return null;
            ...
            // Create script assembly
            ScriptAssembly scriptAssembly = new ScriptAssembly(this, assembly);
            ...
            if (performSecurityCheck == true)
            {
                ...
                {
                    // Log the error
                    RoslynCSharp.LogError(scriptAssembly.SecurityChecker.Report.ToString());
                    // Dont load the assembly
                    return null;
                }
                else
                    RoslynCSharp.Log(scriptAssembly.SecurityChecker.Report.ToString());
            }
