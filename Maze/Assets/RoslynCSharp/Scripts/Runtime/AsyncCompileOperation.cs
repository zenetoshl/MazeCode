...
namespace RoslynCSharp
{
    ...
    public sealed class AsyncCompileOperation : AsyncOperation
    {
        ...
        // Constructor
        internal AsyncCompileOperation(ScriptDomain domain, bool isCSharpSource, ScriptSecurityMode securityMode, params string[] sourceOrFiles)
        {
                    ...
                    case CompileType.CompileSource:
                        {
                            ...
                            {
                                // Compile the source
                                result = compileDomain.CompileAndLoadSource(sourceOrFiles[0], securityMode);
                            }
                            else
                            {
                                // Compile the sources
                                result = compileDomain.CompileAndLoadSources(sourceOrFiles, securityMode);
                            }
                            break;
                        }
                    ...
                    case CompileType.CompileFile:
                        {
                            ...
                            {
                                // Compile the file
                                result = compileDomain.CompileAndLoadFile(sourceOrFiles[0], securityMode);
                            }
                            else
                            {
                                // Compile the files
                                result = compileDomain.CompileAndLoadFiles(sourceOrFiles, securityMode);
                            }
                            break;
                        }
                }
